...
report_mouse_t pointing_device_task_user(report_mouse_t mouse_report) {
    // Mouse movement is detected -> send scroll_lock now
    static uint16_t scroll_timer;
    if (mouse_report.x || mouse_report.y) {
        scroll_timer = timer_read();
        // no scroll lock is enabled
        if(!host_keyboard_led_state().scroll_lock) {
            scroll_lock_state = true;
            tap_code(KC_SCROLL_LOCK);
        }
    }

    if (scroll_lock_state && timer_elapsed(scroll_timer) > SCROLL_TIMEOUT) {
        scroll_lock_state = false;
        if (host_keyboard_led_state().scroll_lock) {
            tap_code(KC_SCROLL_LOCK);
        }
    }
